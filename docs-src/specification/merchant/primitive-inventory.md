...
<div class="diagram"><img src="/documentation/assets/diagrams/m-inventory-model.svg" alt="库存三层数量视图：available 权威值内含 active_holds 占用与 sellable 派生量，发货核销转入 consumed；下半部为 InventoryHold 生命周期 HELD/LOCKED/CONSUMED/RELEASED" style="max-width: 100%; height: auto;"></div>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>consumed</code>（已核销）</td><td>协议引擎（MP4）</td><td><code>shipment.ship</code> 确认后由 lock 转入</td></tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>INVENTORY.REVISION_CONFLICT</code></td><td>error</td><td>409</td><td><code>expected_revision</code> 与当前不一致（并发写入）。</td><td><code>query</code> 重读后重试；Bridge 场景见 M8.5。</td></tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>及时同步</td><td>MUST</td><td>线下渠道或多平台销售导致实际库存变化时，MUST 及时下调；SHOULD 通过 Bridge 事件驱动同步（M8.4）。</td></tr>
    ...
    <ol>
      ...
      <li><strong>核销映射：</strong>MP4 <code>shipment.ship</code> 确认后，对应 <code>LOCKED</code> 数量迁移至 <code>CONSUMED</code>，<code>available</code> 同步扣减（M4.2）。</li>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>source_ref</code></td><td>string</td><td>否</td><td>来源单据引用（ERP 出入库单号，供 M8 对账）。</td></tr>
...
最终 available 一致收敛；对账以 revision 流水 + source_ref 核对（M8.6）。
</code></pre>
