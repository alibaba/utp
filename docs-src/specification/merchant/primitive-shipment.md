...
<h1 id="s-m6">M6 · MP4 发货执行原语（Shipment）</h1>
    ...
    <ul>
      ...
      <li><a href="#s-m66">M6.6 Mode-Driven Behavior（模式驱动行为）</a></li>
      <li><a href="#s-m67">M6.7 与 P5 Fulfill 的事实传导契约</a></li>
      <li><a href="#s-m68">M6.8 Operations 与 Transport Bindings</a></li>
      <li><a href="#s-m69">M6.9 Entities（实体定义）</a></li>
      <li><a href="#s-m610">M6.10 Use Case Walkthroughs（用例演练）</a></li>
    </ul>
    <hr />
...
service:        dev.utp.merchant
    ...
    <ul>
      ...
      <li><strong>备货态是供应商内部状态的最小外露。</strong>主规范明确"备货、待发等供应方内部状态不在（买方可观测）状态机范围内"（15.3.1）。MP4 的 <code>prepare</code> 仅用于供应商向平台申报备货进度（供 <code>leadtime</code> 查询参考与延迟预警），MUST NOT 触发买方可观测状态迁移。</li>
      <li><strong>验收永远在买方。</strong><code>receive</code>/<code>reject</code>/<code>inspect</code> 是 P5 的买方/检验方动作；MP4 不定义任何验收或逆向物流动作，退换货走 P6 Resolve 的补偿指令。</li>
    </ul>
    ...
    <table>
      ...
      <tbody>
        <tr><td><code>purchase.status == 'PURCHASED'</code></td><td>MUST</td><td>订购已成立（MP3 ACCEPTED 且 P3 原子迁移完成）。</td></tr>
...
<pre class="highlight"><code class="language-json">{
  ...
  "compensation": {
    ...
    "on_buyer_reject": "P5 reject → P6 Resolve 补偿指令决定退货/退款；MP4 只读关联"
  }
}
</code></pre>
...
<div class="diagram"><img src="/documentation/assets/diagrams/m-shipment-state-machine.svg" alt="Shipment 资源状态机：PREPARING/READY/SHIPPED/EXCEPTION/DELIVERED/CLOSED 及买方可观测映射" style="max-width: 100%; height: auto;"></div>
    ...
    <table>
      ...
      <tbody>
        <tr><td><code>SHIPMENT.ORDER_NOT_PURCHASED</code></td><td>error</td><td>409</td><td>订购未成立（非 <code>PURCHASED</code>）即尝试发货。</td><td>等待 MP3 受理与 P3 成立。</td></tr>
    ...
    <p><strong>约束：</strong><code>ship</code> 的 Shipment 凭证 MUST 携带 Seller ES256 签名；当某段履约责任由独立物流方承担时，承运段的轨迹事实由 Shipper 按主规范 <code>Fulfill.track</code> 义务提交，MP4 不代替 Shipper 签名。</p>
    ...
    <table>
      ...
      <tbody>
        <tr><td>按期发货</td><td>MUST</td><td>MUST 在承诺交期（Listing <code>leadtime_days</code> 或 MP3 修订值）内 <code>ship</code>；预见延迟 MUST 提前 <code>update(delay)</code> 申报。</td></tr>
    ...
    <table>
      <thead><tr><th>职责</th><th>级别</th><th>说明</th></tr></thead>
      <tbody>
        <tr><td>事实转换</td><td>MUST</td><td>MP4 生效事件 MUST 在声明时限（SHOULD ≤ 60s）内转换为买方侧 <code>fulfill.notify</code> 推送与 TrackingEvent（M6.7 映射表）。</td></tr>
    ...
    <hr />
    <h2 id="s-m66">M6.6 Mode-Driven Behavior（模式驱动行为）</h2>
    <table>
      <thead><tr><th>Mode 配置</th><th>MP4 行为</th></tr></thead>
      <tbody>
        ...
        <tr><td>fulfillment=L2（分阶段履约）</td><td>阶段性发货事实和凭证 MUST 可关联到所属履约阶段；<code>split</code> 仅在交易条款允许时可用；某阶段要求独立检验时，<code>INSPECTED</code> 结论由 Inspector 提交（P5），MP4 只读。</td></tr>
        ...
        <tr><td>payment=L1+（分期/里程碑）</td><td><code>ship</code>/<code>DELIVERED</code>/检验合格等事实按 TradeMethod 子步骤 MAY 触发下一期支付条件；MP4 响应 MUST 回显所触发的子步骤标识。</td></tr>
    ...
    <hr />
    <h2 id="s-m67">M6.7 与 P5 Fulfill 的事实传导契约（Interlock with P5）</h2>
    <p>本节是闭环不变式 4（<a href="overview.html#s-m16">M1.6</a>）的规范定义。Marketplace MUST 按下表执行 MP4 → P5 的事实转换：</p>
    <table>
      <thead><tr><th>MP4 事件</th><th>买方侧 <code>fulfill.notify</code></th><th>TrackingEvent.status</th><th>买方可观测状态</th></tr></thead>
      <tbody>
    ...
    <hr />
    <h2 id="s-m68">M6.8 Operations 与 Transport Bindings</h2>
    <h3 id="s-m681">M6.8.1 操作矩阵</h3>
    <table>
    ...
    <hr />
    <h2 id="s-m69">M6.9 Entities（实体定义）</h2>
    <h3 id="s-m691">M6.9.1 Shipment（发货凭证）</h3>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>status</code></td><td>enum</td><td>是</td><td>M6.2.2 状态枚举。</td></tr>
        ...
        <tr><td><code>packages</code></td><td>Package[]</td><td>是</td><td>包裹明细（M6.9.2），至少一件。</td></tr>
    ...
    <h3 id="s-m692">M6.9.2 Package</h3>
    <table>
      <thead><tr><th>字段名</th><th>类型</th><th>必填</th><th>描述</th></tr></thead>
      <tbody>
    ...
    <h3 id="s-m693">M6.9.3 ShipmentEvent（异常/补充事件）</h3>
    <table>
      <thead><tr><th>字段名</th><th>类型</th><th>必填</th><th>描述</th></tr></thead>
      <tbody>
    ...
    <h3 id="s-m694">M6.9.4 SplitPlan</h3>
    ...
    <hr />
    <h2 id="s-m610">M6.10 Use Case Walkthroughs（用例演练）</h2>
    <h3 id="s-m6101">M6.10.1 标准发货与买方感知</h3>
...
//   "status": "SHIPPED", "tracking_number": "SF1234567890123", ... }
    ...
    <h3 id="s-m6102">M6.10.2 延迟申报与分批发货</h3>
