...
| `utp.purchase.contract_create` | `utp_purchase_contract_create` | 输入为 ContractCreateRequest。 |
| `utp.purchase.contract_update` | `utp_purchase_contract_update` | 输入为 ContractUpdateRequest。 |
| `utp.purchase.contract_complete` | `utp_purchase_contract_complete` | 输入为 ContractCompleteRequest（含 Mandate）。 |
...
- 除 `create` 与 `contract_create` 外，MCP 工具 MUST 通过 `target` 参数携带唯一的 `purchase_id` 或 `agreement_id`；`query` 与 `cancel` 不得额外定义业务请求体。
