...
<h1 id="s-m8">M8 · 结算与对账（Settlement &amp; Reconciliation）</h1>

    <h2 id="s-m81">M8.1 定位与设计选择（Positioning）</h2>
    ...
    <ol>
      ...
      <li>差异申报（M8.5）虽是写操作，但其语义是"对账异议登记"，不改变任何支付状态；异议的资金后果 MUST 经 P4 <code>refund</code> 或 P6 Resolve 产生。</li>
    </ol>
    ...
    <h2 id="s-m82">M8.2 结算模型（Settlement Model）</h2>
    <h3 id="s-m821">M8.2.1 结算要素</h3>
    <table>
    ...
    <h3 id="s-m822">M8.2.2 结算金额构成不变式</h3>
...
4. Σ(entries.payable) == statement.total_payable
    ...
    <h2 id="s-m83">M8.3 结算条目生命周期（Entry Lifecycle）</h2>
<div class="diagram"><img src="/documentation/assets/diagrams/m-entry-lifecycle.svg" alt="结算条目生命周期：SETTLEABLE 经账单出账至 BILLED、确认后 PAID_OUT；争议冻结 FROZEN 裁决后回归；打款失败 PAYOUT_FAILED 重试" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m84">M8.4 账单出具与查询（Statements）</h2>
    <ul>
      ...
      <li>供应商 MAY 在异议期（SHOULD ≥ 7 天）内提交差异申报（M8.5）；异议期满未申报视为确认。</li>
      <li>账单与打款流水 MUST 归档，保存期不低于所在法域的财务凭证要求与争议时效期两者较长者。</li>
    </ul>
    <h3 id="s-m841">M8.4.1 操作与传输绑定</h3>
    <table>
    ...
    <h2 id="s-m85">M8.5 对账闭环与差异处理（Reconciliation Loop）</h2>
<div class="diagram"><img src="/documentation/assets/diagrams/m-reconciliation-loop.svg" alt="对账闭环：三方核对后一致确认、不一致逐条差异申报，平台 SLA 内答复更正或维持，仍有异议升级 P6 Resolve" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m86">M8.6 实体定义（Entities）</h2>
    <h3 id="s-m861">M8.6.1 SettlementStatement</h3>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>total_gross</code> / <code>total_fees</code> / <code>total_payable</code></td><td>Money</td><td>是</td><td>汇总金额；MUST 满足 M8.2.2 不变式。</td></tr>
    ...
    <h3 id="s-m862">M8.6.2 SettlementEntry</h3>
    <table>
      <thead><tr><th>字段名</th><th>类型</th><th>必填</th><th>描述</th></tr></thead>
      <tbody>
        ...
        <tr><td><code>status</code></td><td>enum</td><td>是</td><td>M8.3 条目状态枚举。</td></tr>
      </tbody>
    </table>
    <h3 id="s-m863">M8.6.3 SettlementDiscrepancy</h3>
    <table>
      <thead><tr><th>字段名</th><th>类型</th><th>必填</th><th>描述</th></tr></thead>
      <tbody>
    ...
    <h2 id="s-m87">M8.7 错误码（Error Codes）</h2>
    <table>
    ...
    <p>结算数据含资金敏感信息：服务端 MUST 仅返回调用方本方数据；Merchant Agent 访问结算数据 MUST 持有覆盖 <code>pay:settlement:read</code> 的显式授权（M10.4），且 SHOULD 在授权中单列（不随经营类 Scope 默认捆绑）。</p>

    <hr />
    <h2 id="s-m89">M8.9 打款动作（Payout）</h2>
    <p>打款是资金流的最后一跳：平台（代收主体，经 PaymentProcessor）将已确认账单的 <code>total_payable</code> 打至供应商收款账户。<strong>协议定位：打款不是新原语</strong>——它是 P4 Pay（意图：执行资金转移）在结算扩展 <code>utp.pay.settlement</code> 内的<strong>平台驱动 Action</strong>，对应条目生命周期（M8.3）的 <code>BILLED → PAID_OUT</code> 迁移；在全局 DAG 中挂接在买方确认收货（P5 receive）与账单确认之后，作为交易闭环的收尾节点。</p>
    <table>
      ...
      <tbody>
        ...
        <tr><td>执行语义</td><td>每次打款 MUST 生成 <code>PayoutInstruction</code>（含 <code>statement_id</code>、金额、收款账户摘要、发起时间）；成功后 MUST 回写 <code>payout_ref</code>（支付渠道流水）并推送 <code>utp.settlement.paid</code> 回调；失败进入 <code>PAYOUT_FAILED</code>，MUST 通知供应商并重试（M8.3）。</td></tr>
