...
<h1 id="s-m9">M9 · 供应商 Agent 与人机协同（Merchant Agent &amp; Human-Agent Interaction）</h1>
    ...
    <table>
      ...
      <tbody>
        <tr><td>入驻资料与资质维护</td><td>在授权下代办资料提交、资质更新与换证（M2.5）；平台账号注册与商户协议签署除外（M2.4.3 / M9.5）</td><td>入驻管理接口（非原语）</td></tr>
        ...
        <tr><td>询盘与报价</td><td>响应买方 P2 Negotiate：<code>delegation_policy</code> 为 <code>passthrough</code> 时接收 <code>inquiry_routed</code> 回调，依据定价策略生成 <code>quote</code>/还盘并回传；框架协议客户按协议价自动报价</td><td>P2（Seller 作为 handler 侧的应答方，执行模式见 M1.4.4）</td></tr>
        <tr><td>接单决策</td><td>按 AcceptancePolicy 自动接单/拒单/挂起/申报交期</td><td>MP3</td></tr>
        <tr><td>履约调度</td><td>备货申报、选择承运商、发货上报、延迟预警与申报</td><td>MP4</td></tr>
        <tr><td>结算核对</td><td>三方对账（M7.5）、差异申报草案生成</td><td>settlement 扩展</td></tr>
        <tr><td>争议应对</td><td>整理证据包、生成答辩草案</td><td>P6（提交前 MUST 人工确认，见 M9.5）</td></tr>
      </tbody>
    </table>

    <h2 id="s-m93">M9.3 自动决策策略（AcceptancePolicy 与定价策略）</h2>
    <h3 id="s-m931">M9.3.1 AcceptancePolicy 实体</h3>
    <p>自动接单策略是 MP3 <code>acceptance_policy.mode == "auto"</code>（M5.6/M5.8）的供应商侧配置。策略 MUST 以结构化形式定义、版本化存档，使每次自动决策可回溯到策略版本：</p>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>auto_reject_rules</code></td><td>array</td><td>否</td><td>自动拒单条件组（命中即 reject，附映射的 M5.10.2 原因码）。</td></tr>
        ...
        <tr><td><code>amount_ceiling</code></td><td>Money</td><td>是</td><td>自动决策金额上限；超过 MUST 升级人工（M9.5 控制点）。</td></tr>
        <tr><td><code>owner_signature</code></td><td>string</td><td>是</td><td>Principal（商家授权人）对策略版本的 ES256 签名——策略即授权的书面化。</td></tr>
      </tbody>
    </table>
    <h3 id="s-m932">M9.3.2 报价策略约束</h3>
    <p>Merchant Agent 代表 Seller 应答 P2 Negotiate 时：</p>
    <ul>
      ...
      <li>生成的 Quote/Binding Terms 遵循主规范 P2 的实体与签名要求；Agent 签名的授权要求见 M9.4。</li>
...
<div class="diagram"><img src="../../assets/diagrams/m-agent-authorization.svg" alt="Agent 授权链：Principal 主密钥签发 Mandate（Scope/金额/时效）授予 Agent 工作密钥，签名动作带 decided_by 进入审计，可回溯可吊销" style="max-width: 100%; height: auto;"></div>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>接单金额超 <code>amount_ceiling</code> / 低毛利 / 黑名单区域</td><td>CONFIRMED</td><td>受理任务 <code>hold</code>，等待人工结论；MUST 在 M5 deadline 内完成，否则超时策略兜底。</td></tr>
    ...
    <h2 id="s-m97">M9.7 Agent 失效与降级（Failure &amp; Fallback）</h2>
    <table>
      <thead><tr><th>失效场景</th><th>降级行为</th></tr></thead>
      <tbody>
        <tr><td>Agent 不可用（宕机/授权过期）</td><td>回调事件积压由 Bridge 队列承接；受理任务按 M5 <code>timeout_policy</code> 兜底；Principal SHOULD 收到不可用告警并可切换人工后台。</td></tr>
        <tr><td>策略配置错误（异常批量拒单/异常报价）</td><td>Principal MAY 紧急吊销授权并回滚策略版本；已产生的承诺性签名不可撤销，善后走 P6。</td></tr>
        <tr><td>Agent 与 ERP 数据不一致</td><td>以 ERP 为南向权威、协议凭证为北向事实，按 M8.7 对账钩子收敛；决策暂降级为 <code>assisted</code>。</td></tr>
      </tbody>
    </table>
