...
<h1 id="s-m11">M11 · 供应商 Agent 与人机协同（Merchant Agent &amp; Human-Agent Interaction）</h1>
    ...
    <h2 id="s-m111">M11.1 定位（Positioning）</h2>
    ...
    <h2 id="s-m112">M11.2 Merchant Agent 职责边界（Responsibility Boundary）</h2>
    <table>
      <thead><tr><th>职责域</th><th>Agent 可执行的动作</th><th>对应原语</th></tr></thead>
      <tbody>
        <tr><td>入驻资料与资质维护</td><td>在授权下代办资料提交、资质更新与换证（M2.5）；平台账号注册与商户协议签署除外（M2.4.3 / M11.5）</td><td>入驻管理接口（非原语）</td></tr>
        ...
        <tr><td>结算核对</td><td>三方对账（M9.5）、差异申报草案生成</td><td>settlement 扩展</td></tr>
        <tr><td>争议应对</td><td>整理证据包、生成答辩草案</td><td>P6（提交前 MUST 人工确认，见 M11.5）</td></tr>
      </tbody>
    </table>

    <h2 id="s-m113">M11.3 自动决策策略（AcceptancePolicy 与定价策略）</h2>
    <h3 id="s-m1131">M11.3.1 AcceptancePolicy 实体</h3>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>amount_ceiling</code></td><td>Money</td><td>是</td><td>自动决策金额上限；超过 MUST 升级人工（M11.5 控制点）。</td></tr>
        <tr><td><code>owner_signature</code></td><td>string</td><td>是</td><td>Principal（商家授权人）对策略版本的 ES256 签名——策略即授权的书面化。</td></tr>
      </tbody>
    </table>
    <h3 id="s-m1132">M11.3.2 报价策略约束</h3>
    ...
    <ul>
      ...
      <li>生成的 Quote/Binding Terms 遵循主规范 P2 的实体与签名要求；Agent 签名的授权要求见 M11.4。</li>
      <li>金额或折扣超出策略边界的还盘 MUST 升级人工确认（SUSPENDED 控制点）。</li>
    </ul>

    <h2 id="s-m114">M11.4 Agent 授权链（Delegation &amp; Mandate）</h2>
<div class="diagram"><img src="/documentation/assets/diagrams/m-agent-authorization.svg" alt="Agent 授权链：Principal 主密钥签发 Mandate（Scope/金额/时效）授予 Agent 工作密钥，签名动作带 decided_by 进入审计，可回溯可吊销" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m115">M11.5 人机控制点（HAI Control Points）</h2>
    ...
    <h2 id="s-m116">M11.6 决策审计（Decision Audit）</h2>
    ...
    <h2 id="s-m117">M11.7 Agent 失效与降级（Failure &amp; Fallback）</h2>
    <table>
      ...
      <tbody>
        ...
        <tr><td>Agent 与 ERP 数据不一致</td><td>以 ERP 为南向权威、协议凭证为北向事实，按 M10.7 对账钩子收敛；决策暂降级为 <code>assisted</code>。</td></tr>
      </tbody>
    </table>
