...
<h1 id="s-m10">M10 · 供应商 Agent 与人机协同（Merchant Agent &amp; Human-Agent Interaction）</h1>
    ...
    <h2 id="s-m102">M10.2 Merchant Agent 职责边界（Responsibility Boundary）</h2>
    <table>
      <thead><tr><th>职责域</th><th>Agent 可执行的动作</th><th>对应原语</th></tr></thead>
      <tbody>
        <tr><td>入驻资料与资质维护</td><td>在授权下代办资料提交、资质更新与换证（M2.5）；平台账号注册与商户协议签署除外（M2.4.3 / M10.5）</td><td>入驻管理接口（非原语）</td></tr>
        ...
        <tr><td>询盘与报价</td><td>响应买方 P2 询盘经 MP3 应答（<a href="primitives/quote/index.html">M5</a>）：<code>delegation_policy</code> 为 <code>passthrough</code> 时接收 <code>inquiry_routed</code> 回调，依据定价策略生成 <code>quote</code>/还盘并回传；框架协议客户按协议价自动报价</td><td>P2（Seller 作为 handler 侧的应答方，执行模式见 M1.4.4）</td></tr>
        ...
        <tr><td>结算核对</td><td>三方对账（M8.5）、差异申报草案生成</td><td>settlement 扩展</td></tr>
        <tr><td>争议应对</td><td>整理证据包、生成答辩草案</td><td>P6（提交前 MUST 人工确认，见 M10.5）</td></tr>
      </tbody>
    </table>

    <h2 id="s-m103">M10.3 自动决策策略（AcceptancePolicy 与定价策略）</h2>
    <h3 id="s-m1031">M10.3.1 AcceptancePolicy 实体</h3>
    <p>自动接单策略是 MP4 <code>acceptance_policy.mode == "auto"</code>（M6.6/M6.8）的供应商侧配置。策略 MUST 以结构化形式定义、版本化存档，使每次自动决策可回溯到策略版本：</p>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>auto_reject_rules</code></td><td>array</td><td>否</td><td>自动拒单条件组（命中即 reject，附映射的 M6.10.2 原因码）。</td></tr>
        ...
        <tr><td><code>amount_ceiling</code></td><td>Money</td><td>是</td><td>自动决策金额上限；超过 MUST 升级人工（M10.5 控制点）。</td></tr>
        <tr><td><code>owner_signature</code></td><td>string</td><td>是</td><td>Principal（商家授权人）对策略版本的 ES256 签名——策略即授权的书面化。</td></tr>
      </tbody>
    </table>
    <h3 id="s-m1032">M10.3.2 报价策略约束</h3>
    <p>Merchant Agent 代表 Seller 经 MP3 应答 P2 询盘时（M5）：</p>
    <ul>
      ...
      <li>生成的 Quote/Binding Terms 遵循主规范 P2 的实体与签名要求；Agent 签名的授权要求见 M10.4。</li>
      <li>金额或折扣超出策略边界的还盘 MUST 升级人工确认（SUSPENDED 控制点）。</li>
    </ul>

    <h2 id="s-m104">M10.4 Agent 授权链（Delegation &amp; Mandate）</h2>
<div class="diagram"><img src="/documentation/assets/diagrams/m-agent-authorization.svg" alt="Agent 授权链：Principal 主密钥签发 Mandate（Scope/金额/时效）授予 Agent 工作密钥，签名动作带 decided_by 进入审计，可回溯可吊销" style="max-width: 100%; height: auto;"></div>
    <p>Merchant Agent 的每次签名动作 MUST 可回溯到 Principal 授权，复用主规范 Ch.6 的身份与授权框架（Agent 身份凭证 + 授权链），供应商侧的授权要素：</p>
    <table>
      <thead><tr><th>要素</th><th>要求</th></tr></thead>
      <tbody>
        <tr><td>授权主体</td><td>Principal（商家法人/授权经营者）以 Profile 主密钥签发对 Agent 工作密钥的授权凭证。凭证类型 MUST 为主规范 <code>operation</code> Mandate（Profile <code>mandates.supported_mandate_types</code> 中的既有类型，不新增类型），结构与验证规则同主规范第 6 章；供应商侧仅约束其 Scope、金额与时效内容如下。</td></tr>
        <tr><td>授权范围</td><td>MUST 按 Scope 枚举（附录 MC）：如 <code>listing:*</code>、<code>inventory:*</code>、<code>acceptance:accept</code>（含金额上限）、<code>shipment:*</code>；结算读取 <code>pay:settlement:read</code> SHOULD 单独授予。</td></tr>
    ...
    <h2 id="s-m105">M10.5 人机控制点（HAI Control Points）</h2>
    <p>复用主规范 Ch.20 的三级交互控制等级（AUTONOMOUS &lt; SUPERVISED &lt; CONFIRMED，<a href="/documentation/specification/protocol-core/human-agent-interaction.html#s-1912">20.1.2</a>：自动执行 / 监督执行（非阻断，倒计时窗口内可人工撤回）/ 人工确认（阻断执行门））与 HAI 挂起语义（Suspend Record，20.7.2；执行前控制，17.1.6.6），定义供应商侧默认控制点矩阵：</p>
    <table>
      ...
      <tbody>
        ...
        <tr><td>询盘报价（策略内，MP3）</td><td>AUTONOMOUS</td><td>按报价策略自动 quote 并签名（M5）；策略外或超授权限额升级 CONFIRMED。</td></tr>
    ...
    <p><strong>与买方侧 HAI 挂起的关系：</strong>供应商侧人工确认发生在 MP4 受理窗口内（<code>ON_HOLD</code> 状态），属于供应商内部流程，<strong>不产生</strong>买方会话的 HAI 挂起记录（Suspend Record）——后者是买方 Principal 对待确认 Action 的控制语义（主规范 20.7.2，执行前控制见 17.1.6.6）。两侧时限的衔接由 M6 的 <code>deadline</code> 统一约束。</p>

    <h2 id="s-m106">M10.6 决策审计（Decision Audit）</h2>
    <p>本节审计要求继承主规范<a href="/documentation/specification/protocol-core/risk-audit.html">第 7 章</a>（风控与审计）：审计记录结构、保留期限与风控信号采集规则以该章为准，本节只补充供应商侧的决策来源维度。</p>
    <ul>
      ...
      <li>Marketplace 对 <code>decided_by == 'agent' | 'policy_auto'</code> 的决策 MAY 施加额外风控采样（主规范 Ch.7 风控信号框架）；供应商 MUST NOT 将人工决策伪标为自动或反之。</li>
    </ul>

    <h2 id="s-m107">M10.7 Agent 失效与降级（Failure &amp; Fallback）</h2>
    <table>
      ...
      <tbody>
        ...
        <tr><td>Agent 与 ERP 数据不一致</td><td>以 ERP 为南向权威、协议凭证为北向事实，按 M9.7 对账钩子收敛；决策暂降级为 <code>assisted</code>。</td></tr>
      </tbody>
    </table>
