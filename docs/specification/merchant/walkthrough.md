...
<h1 id="s-m10">M10 · 供应商全链路演练（Merchant End-to-End Walkthrough）</h1>
...
<div class="diagram"><img src="../../assets/diagrams/m-e2e-sequence.svg" alt="供应商全链路时序总览：Phase 0—6 从入驻、发布、被寻源、报价、接单、发货到结算的三方交互" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m103">M10.3 Phase 1：发布商品与设置库存</h2>
...
3. PUT  /utp/m/v1/inventory/item-BT-NC-001/sku-X3-BLK
   ...
   → { available:500, sellable:500, revision:1 }
    ...
    <h2 id="s-m104">M10.4 Phase 2—3：被寻源与报价（买方视角对照）</h2>
...
// Merchant Agent 按 M9.3.2 策略生成报价（阶梯价内，无需人工）：
// negotiate.quote 应答
{
  "quote": {
  ...
  "decided_by": "agent"
    ...
    <h2 id="s-m105">M10.5 Phase 4：下单、路由与接单</h2>
...
//    hold-20260722-0091: HELD → LOCKED；ERP 创建 SO-2026-07553 并建立映射
    ...
    <p><strong>闭环检查点 ②（不变式 3）：</strong>MP3 提交的卖方签名覆盖订购 <code>terms_hash</code>，与买方侧 Mandate 操作准入共同构成双边承诺，订购凭证成立，全局状态 <code>PURCHASING → PAYING</code>。买方支付（P4，全额 alipay）确认后进入 <code>FULFILLING</code>。</p>

    <h2 id="s-m106">M10.6 Phase 5：发货与买方收货</h2>
...
// 2. Marketplace 转换为买方 fulfill.notify(SHIPPED)（M6.7 映射表）
...
// → 批次 CLOSED；全局状态 FULFILLING → SETTLED（全局状态机判定）
    ...
    <h2 id="s-m107">M10.7 Phase 6：结算与对账</h2>
...
// → PAID_OUT（附打款流水 payout-20260803-771）→ ERP 应收核销
    ...
    <p><strong>闭环检查点 ④（不变式 M7.2.2）：</strong><code>gross_amount</code> 与 PaymentConfirmation 一致；每笔扣减有协议条款引用；账单可逐笔回溯到 <code>transaction_id</code>。至此货、款、单三流闭合。</p>

    <h2 id="s-m108">M10.8 异常支线演练（Exception Paths）</h2>
    <table>
      ...
      <tbody>
        ...
        <tr><td>买方拒收</td><td>到货外观破损</td><td>买方 <code>fulfill.reject</code>（P5）→ Resolve 激活 → 裁决退款 → CompensationOrder 进入结算 <code>adjustments</code>（M7.6.2）</td><td>结算条目冻结 → 裁决后按调整项出账</td></tr>
    ...
    <h2 id="s-m109">M10.9 闭环总验证清单（Loop Verification Checklist）</h2>
    <table>
      <thead><tr><th>#</th><th>不变式（M1.6）</th><th>本章验证位置</th></tr></thead>
      <tbody>
        <tr><td>1</td><td>Source 投影 == Listing 发布结构</td><td>M10.3 检查点 ①</td></tr>
        <tr><td>2</td><td>Purchase 库存校验/锁定 == MP2 同一数据源</td><td>M10.5（hold→LOCKED→CONSUMED 全程可查）</td></tr>
        <tr><td>3</td><td>卖方承诺处理经 MP3 完成并注入 P3 原子迁移</td><td>M10.5 检查点 ②</td></tr>
        <tr><td>4</td><td>MP4 Shipment == 买方 fulfill 同一记录</td><td>M10.6 检查点 ③</td></tr>
        <tr><td>5</td><td>delist 后 Source/Purchase 既有错误码生效</td><td>M3.10.2（专项演练）</td></tr>
        <tr><td>—</td><td>结算三流闭合（货/款/单）</td><td>M10.7 检查点 ④</td></tr>
      </tbody>
    </table>
