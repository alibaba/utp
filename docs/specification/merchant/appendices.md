...
<h1 id="s-m-appendices">UTP-M 附录（Appendices）</h1>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>平台方</td><td>Marketplace</td><td>托管商品目录与订单路由的 UTP Server 运营方角色，MP1—MP6 原语的 handler_role。</td><td>M1.3.1</td></tr>
        ...
        <tr><td>打款</td><td>Payout</td><td>平台将已确认账单应付金额打至供应商收款账户的平台驱动动作（P4 结算扩展内，非原语）。</td><td>M9.9</td></tr>
        ...
        <tr><td>受理策略</td><td>AcceptancePolicy</td><td>Principal 签名的自动接单策略配置，自动决策的授权书面化。</td><td>M11.3.1</td></tr>
        ...
        <tr><td>结算账单</td><td>SettlementStatement</td><td>Marketplace 按结算周期出具的签名账单，逐笔可追溯到交易与支付凭证。</td><td>M9.6.1</td></tr>
        <tr><td>差异申报</td><td>Settlement Discrepancy</td><td>供应商对账单条目的结构化异议登记；资金后果经 P4/P6 产生。</td><td>M9.6.3</td></tr>
        <tr><td>商户桥接器</td><td>Merchant Bridge</td><td>连接供应商内部系统（ERP/WMS）与 UTP-M 原语的集成组件。</td><td>M10.1</td></tr>
        <tr><td>供应商代理</td><td>Merchant Agent</td><td>代表供应商 Principal 自主执行经营决策的 AI Agent；协议层以 Seller 身份出现。</td><td>M11.1</td></tr>
        <tr><td>渠道配额</td><td>Channel Quota</td><td>ERP 分配给单一销售渠道的库存份额；平台 <code>available</code> 是其镜像。</td><td>M10.5</td></tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>ACCEPTANCE.*</code></td><td>10</td><td><a href="primitives/acceptance/index.html#s-m63">M6.3</a></td><td><code>ACCEPTANCE.EXPIRED</code>、<code>ACCEPTANCE.SIGNATURE_INVALID</code>、<code>ACCEPTANCE.AMEND.OUT_OF_RANGE</code></td></tr>
    ...
    <table>
      ...
      <tbody>
        <tr><td><code>listing</code> / <code>inventory</code> / <code>quote</code> / <code>acceptance</code> / <code>delivery</code> / <code>aftersale</code></td><td>Primitive 级聚合</td><td>MP1—MP6（Profile <code>primitives[].authorization.scope</code> 声明用，蕴含该原语全部 Action Scope）</td><td>M2.3</td></tr>
        ...
        <tr><td><code>delivery:prepare</code> / <code>delivery:ship</code> / <code>delivery:split</code> / <code>delivery:update</code> / <code>delivery:query</code></td><td>Action</td><td>MP5 <code>utp.delivery</code></td><td>M7.4</td></tr>
        <tr><td><code>aftersale:write</code>（approve / reject / propose / confirm_return） / <code>aftersale:read</code>（query / list）</td><td>Action</td><td>MP6 <code>utp.aftersale</code></td><td>M8.7</td></tr>
        <tr><td><code>pay:settlement:read</code></td><td>Data</td><td><code>utp.pay.settlement</code> 扩展</td><td>M9.8</td></tr>
        <tr><td><code>pay:settlement:discrepancy</code></td><td>Action</td><td><code>utp.pay.settlement</code> 扩展</td><td>M9.8</td></tr>
      </tbody>
    </table>
    <p>全部 Scope 仅作用于调用方本方资源；Merchant Agent 持有的授权 MUST 显式枚举 Scope 并声明金额上限与时效（M11.4）。</p>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>8</td><td>M9 结算扩展</td><td>primitive-pay.html 新增"settlement 扩展"小节（按 10.5 扩展规范格式）</td><td>Profile <code>primitives[].extensions</code> 声明说明；错误码表</td></tr>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>2</td><td><code>fulfill.ship</code> 的既有不一致（ME 第 7 项）</td><td><strong>已解决</strong>：主规范已清除全部 <code>fulfill.ship</code> 残留，Ch.15 保持纯买方视角，发货动作统一由 <code>utp.delivery.ship</code> 承载</td><td>已闭环（保留备忘）</td></tr>
