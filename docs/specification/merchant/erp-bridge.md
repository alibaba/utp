...
<h1 id="s-m10">M10 · ERP 集成与 Bridge（ERP Integration &amp; Merchant Bridge）</h1>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>agent_embedded</code>（Agent 内嵌）</td><td>Merchant Agent（M11）内嵌 Bridge 能力，集成与决策一体</td><td>中小供应商；SaaS 化交付</td></tr>
...
<div class="diagram"><img src="/documentation/assets/diagrams/m-bridge-architecture.svg" alt="Merchant Bridge 架构：北向协议面（MP 调用/回调/签名幂等）与南向集成面（单据订阅/DB/API/MQ），中部 ID 映射表与同步游标" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m104">M10.4 同步模式（Synchronization Patterns）</h2>
    <h3 id="s-m1041">M10.4.1 上行同步（ERP → Marketplace）</h3>
    <table>
    ...
    <h3 id="s-m1042">M10.4.2 下行同步（Marketplace → ERP）</h3>
    <table>
      ...
      <tbody>
        ...
        <tr><td>结算账单</td><td><code>statement_issued</code> 回调（M9.4）</td><td>生成应收对账任务</td></tr>
      </tbody>
    </table>
    <p>回调不可达时按主规范 4.3.2 轮询降级：Bridge MUST 实现基于游标的定时拉取（<code>acceptance.list</code>、<code>inventory.hold.query</code>、<code>delivery.query</code>、<code>settlement.statement.list</code>），确保任何推送丢失都能在一个轮询周期内收敛。</p>

    <h2 id="s-m105">M10.5 库存一致性与防超卖（Inventory Consistency）</h2>
    ...
    <h2 id="s-m106">M10.6 幂等、顺序与容错（Idempotency, Ordering &amp; Fault Tolerance）</h2>
    <ul>
    ...
    <h2 id="s-m107">M10.7 对账钩子（Reconciliation Hooks）</h2>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>单据层</td><td>ERP SO/出库/应收 × 平台受理/发货/结算记录</td><td><code>acceptance.list</code>、<code>delivery.query</code>、<code>settlement.entry.query</code></td><td>差异→补传事实或提交 M9.5 差异申报</td></tr>
      </tbody>
    </table>

    <h2 id="s-m108">M10.8 安全边界（Security Boundary）</h2>
    <ul>
