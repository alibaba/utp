...
<h1 id="s-m9">M9 · ERP 集成与 Bridge（ERP Integration &amp; Merchant Bridge）</h1>
    ...
    <p>本章内容是<strong>实现指引 + 少量互操作约束</strong>：ID 映射（M9.3）与幂等/一致性规则（M9.5）为规范性要求（RFC 关键词有效）；同步模式与部署形态为最佳实践。</p>

    <h2 id="s-m92">M9.2 部署形态（Deployment Patterns）</h2>
    <table>
      ...
      <tbody>
        ...
        <tr><td><code>agent_embedded</code>（Agent 内嵌）</td><td>Merchant Agent（M10）内嵌 Bridge 能力，集成与决策一体</td><td>中小供应商；SaaS 化交付</td></tr>
...
<div class="diagram"><img src="/documentation/assets/diagrams/m-bridge-architecture.svg" alt="Merchant Bridge 架构：北向协议面（MP 调用/回调/签名幂等）与南向集成面（单据订阅/DB/API/MQ），中部 ID 映射表与同步游标" style="max-width: 100%; height: auto;"></div>
    ...
    <h2 id="s-m93">M9.3 ID 映射规范（Identifier Mapping）</h2>
    ...
    <h2 id="s-m94">M9.4 同步模式（Synchronization Patterns）</h2>
    <h3 id="s-m941">M9.4.1 上行同步（ERP → Marketplace）</h3>
    <table>
    ...
    <h3 id="s-m942">M9.4.2 下行同步（Marketplace → ERP）</h3>
    <table>
      <thead><tr><th>数据流</th><th>来源</th><th>ERP 动作</th></tr></thead>
      <tbody>
        <tr><td>订单路由</td><td><code>order_routed</code> 回调（M6.7）</td><td>创建待审 SO / 触发自动审批策略</td></tr>
        <tr><td>库存占用/释放</td><td><code>hold_created</code>/<code>hold_released</code> 回调</td><td>创建/释放库存预留单</td></tr>
        <tr><td>妥投与收货回执</td><td><code>delivery_receipt</code> 回调（M7.7）</td><td>SO 履约完成、确认收入条件</td></tr>
        <tr><td>结算账单</td><td><code>statement_issued</code> 回调（M8.4）</td><td>生成应收对账任务</td></tr>
      </tbody>
    </table>
    ...
    <h2 id="s-m95">M9.5 库存一致性与防超卖（Inventory Consistency）</h2>
    ...
    <h2 id="s-m96">M9.6 幂等、顺序与容错（Idempotency, Ordering &amp; Fault Tolerance）</h2>
    <ul>
    ...
    <h2 id="s-m97">M9.7 对账钩子（Reconciliation Hooks）</h2>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr><td>单据层</td><td>ERP SO/出库/应收 × 平台受理/发货/结算记录</td><td><code>acceptance.list</code>、<code>shipment.query</code>、<code>settlement.entry.query</code></td><td>差异→补传事实或提交 M8.5 差异申报</td></tr>
      </tbody>
    </table>

    <h2 id="s-m98">M9.8 安全边界（Security Boundary）</h2>
    <ul>
