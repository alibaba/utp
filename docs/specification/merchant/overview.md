...
4. **协议并列与分层解耦（Parallel Protocol &amp; Layered Decoupling）：**本规范与 UTP-B 是**并列的独立协议**而非其子集（2026-08-03 分层解耦裁决）。二者共享 UTP 通用底层（商业场景、商业拓扑、发现协商、通信认证、P0 原语框架、六维 Mode 维度模型、通用实体与错误码格式）；但 UTP-B 的两项高级能力——**采购模式编排**（[Ch.8](../protocol-core/procurement-models.md) 中的模式档位组合与模式驱动的流程编排）、**全局状态机**（[Ch.17](../protocol-core/global-state-machine.md)）——为 UTP-B 专属，本规范 MUST NOT 依赖。本规范对这两项能力的引用仅用于**声明边界**（说明 MP 原语不进入其管辖，见 全局状态机边界声明（State Machine Boundary）），而非复用其机制。**路径编排**（[Ch.18](../protocol-core/path-orchestration.md)）属共享底层，但其 `primitive_dag_skeleton` 只编排买方侧 P1—P6；MP 原语 MUST NOT 出现在 DAG 骨架中（见 全局状态机边界声明（State Machine Boundary））。**边界辨析：**Ch.8 定义的**六维 Mode 维度模型**属共享底层（在 Profile 的 `supported_mode_range` 中声明、发现协商时逐维求交集，见 UTP 规范 《发现与协商》），因此 MP 原语复用该模型（见 Mode 对供应商侧的影响（Mode Awareness）、[B2B 与 B2C 场景的改价策略](primitives/acceptance/index.md#s-m6122)）不构成对 UTP-B 专属能力的依赖。
...
| 原语 | 原语 ID | 版本 | initiator_role | handler_role | actions | service |
| --- | --- | --- | --- | --- | --- | --- |
| MP1 商品原语 | `utp.listing` | `2026-07-31` | `Seller` | `Marketplace` | `publish, update, list, delist, query, archive, batch` | `dev.utp.merchant` |
| MP2 库存原语 | `utp.inventory` | `2026-07-31` | `Seller` | `Marketplace` | `set, adjust, query, hold.query, batch` | `dev.utp.merchant` |
| MP3 报价原语（MAY） | `utp.quote` | `2026-07-31` | `Seller` | `Marketplace` | `quote, revise, bid, decline, query, list` | `dev.utp.merchant` |
| MP4 接单原语 | `utp.acceptance` | `2026-07-31` | `Seller` | `Marketplace` | `accept, reject, hold, amend_leadtime, amend_price, query, list` | `dev.utp.merchant` |
| MP5 交付原语 | `utp.delivery` | `2026-07-31` | `Seller` | `Marketplace` | `prepare, ship, split, update, query` | `dev.utp.merchant` |
| MP6 售后原语（新增，MAY） | `utp.aftersale` | `2026-07-31` | `Seller` | `Marketplace` | `approve, reject, propose, confirm_return, query, list` | `dev.utp.merchant` |

六个原语共同挂载在新的 `dev.utp.merchant` Service 上（Profile 的 `utp.services` 中声明，见 [Step 2：Profile 发布（Profile Declaration）](onboarding.md#s-m23)），与 UTP-B 规范 `dev.utp.trade` Service 分离，便于平台按侧别独立部署与限流。B/M 原语对偶不复用命名（例如买方侧 P2 询盘与卖方侧 MP3 报价各自独立声明），同一 Agent Server 可在一份 Profile 中按角色分别声明两侧原语。
...
| --- | --- | --- | --- |
| P1 Source（Ch.11） | `search/lookup` 的数据源 | LISTED 商品 + 实时 `available` 快照 | MUST（平台托管） |
| P2 询盘（Ch.12） | 询盘的卖方侧应答与报价事实 | MP3 询盘响应（quote/revise/bid/decline，M5）；报价口径与 Listing `pricing` 一致（商品原语） | MAY（声明 `utp.quote` 的商户 MUST 按 M5 应答） |
| P3 Purchase（Ch.13） | 库存校验、临时 hold、最终锁定；卖方承诺处理 | MP2 库存视图；MP4 受理结论与签名 | MUST |
| P4 Pay（Ch.14） | 支付确认、Escrow 释放 | M9 结算账单以 PaymentConfirmation 为对账基准 | MUST |
| P5 Fulfill（Ch.15） | `notify` 推送的事实来源 | MP5 的 Shipment 事件（SHIPPED/DELAYED/妥投回执） | MUST |
| P6 Resolve（Ch.16） | 争议证据 | MP1—MP6 全部凭证（ListingSnapshot、AcceptanceRecord、Shipment、AftersaleRecord）MUST 可纳入 Evidence Bundle；**MP6 售后 MUST 先行于 P6 争议**——售后处置记录（含拒绝原因与举证）是争议阶段的证据基础（[与 P6 Resolve 的边界（Aftersale vs Dispute）](primitives/aftersale/index.md#s-m82)） | MUST |

## Mode 对供应商侧的影响（Mode Awareness） {#s-m18}

MP 原语复用 UTP 规范六个 Mode 维度（[Ch.8](../protocol-core/procurement-models.md)），不新增维度。
...
- **操作准入：**Marketplace 处理 MP 写操作前 MUST 按 UTP 规范[第 5 章](../protocol-core/security-trust.md#s-5-trust-profile) 《操作准入语义（Operation Admission Semantics）》 节完成操作准入判定，判定顺序与失败语义同 UTP 规范；认证与授权失败 MUST 使用全局错误码（`UTP.AUTH_UNAUTHORIZED` / `UTP.AUTH_FORBIDDEN`，《全局错误类别》）。Merchant Agent 代理操作时的 Mandate 要求见 Agent 授权链（Delegation & Mandate）。
...
- **响应自描述：**遵循 UTP 规范 [第 19 章](../protocol-core/agent-friendly-interface.md) 自描述响应原则（《总览（Overview）》）：响应 MUST 携带 `valid_next_actions`，元素 MUST 为全限定 Action 名（如 `utp.acceptance.query`；Schema 约束见 `primitives/common/valid_next_actions.json`）。列表操作的分页 MUST 使用游标语义（`cursor`/`limit`，`primitives/common/pagination.json`）。
