...
`error` 的字段定义以 [P0 原语通用框架](../protocol-core/primitive-framework.md#s-1043-standard-error-response) 和 `ErrorResponse` Schema 为准。调用方 MUST 依据 `code` 前缀判定责任层，而不是依据具体传输绑定的状态码、异常或任务状态。

## 错误码空间

| 前缀 | 责任方 | 适用范围 | 权威定义 |
| --- | --- | --- | --- |
| `TRANSPORT.*` | 传输与通信 | 网关认证、信封与上下文校验、防重放、幂等、Service 可用性、投递超时和网关限流 | [传输与通信](../protocol-core/transport-communication.md#s-47) |
| `UTP.*` | P0 原语框架 | 跨原语的通用业务校验、业务授权、业务冲突、原语执行超时、业务能力不可用和业务限流 | [原语框架](../protocol-core/primitive-framework.md#s-104-error-code-framework) |
