...
商业场景用于说明同一组 UTP 原语、Mode 与商业拓扑如何组合成典型采购路径。场景本身不引入新的状态、角色或 Action；一笔交易的实际路径由 Mode、CommerceTopology、已锁定交易条款和对应原语的标准结果共同确定。

本页仅描述 B2C 标准采购与 B2B 标准采购的基线路径。更复杂的跨境、框架协议、竞价或争议场景可在后续页面单独展开。
...
该路径跳过 `NEGOTIATING`，不使用分阶段付款，也不建立父级采购协议。与 B2C 最简路径相比，它增加的是 `compliance_level = L1` 带来的组织主体、采购授权、发票和审计证据要求。
