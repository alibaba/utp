...
- `quote` 是 Seller 生成并签署报价后向 Buyer 交付的异步操作，不是 Buyer 调用的报价接口。Buyer 在 Profile 或会话建立时登记 `callback_base`；Seller MUST 向固定端点 `{callback_base}/utp/v1/negotiate/quote` 发送完整 MessageEnvelope。Buyer 返回 `2xx` 仅表示已接收，不表示接受报价。
