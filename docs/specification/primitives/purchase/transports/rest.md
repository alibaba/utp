...
| `utp.purchase.contract-create` | `POST` | `/utp/v1/purchase/contracts` | `application/json` |
| `utp.purchase.contract-update` | `PATCH` | `/utp/v1/purchase/contracts/{agreement_id}` | `application/json` |
| `utp.purchase.contract-complete` | `POST` | `/utp/v1/purchase/contracts/{agreement_id}/complete` | `application/json` |
...
- `complete` 和 `contract-complete` 支持异步处理；订单或协议目标分别由路径中的 `purchase_id` 或 `agreement_id` 确定，请求体承载完成操作所需的业务输入与 Mandate。
- `update` 与 `contract-update` 使用 `PATCH` 语义，仅传输需要修改的字段。
